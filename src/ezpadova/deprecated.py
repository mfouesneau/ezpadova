...
@deprecated_replacedby("ezpadova.parsec.get_isochrones")
def get_one_isochrone(
    ...
    **kwargs,
    ...
    Returns:
        pd.DataFrame or bytes: The isochrone data, format depends on `ret_table` parameter.
...
def get_Z_isochrones(
    ...
    **kwargs,
    ...
    ----------
    z0 : float
        Lower bound of the metallicity range in Z units.
    z1 : float
        Upper bound of the metallicity range in Z units.
    dz : float
        Step size for the metallicity range in Z units.
    age_yr : float, optional
        Age in years. Either `age_yr` or `logage` must be provided, but not both.
    logage : float, optional
        Logarithm of the age. Either `age_yr` or `logage` must be provided, but not both.
    ret_table : bool, optional
        If True, return the result as a DataFrame. Default is True.
    kwargs : dict
        Additional keyword arguments to be passed to the `get_isochrones` function.
    ...
    ------
    ValueError
        If neither `age_yr` nor `logage` is provided, or if both are provided.
...
def get_t_isochrones(
    ...
    **kwargs,
    ...
    ----------
    logt0 : float
        Lower bound of the age range in log10(years).
    logt1 : float
        Upper bound of the age range in log10(years).
    dlogt : float
        Step size for the age range in log10(years).
    Z : float, optional
        Metallicity. Either `Z` or `MH` must be provided, but not both.
    MH : float, optional
        Metallicity in terms of [M/H]. Either `Z` or `MH` must be provided, but not both.
    ret_table : bool, optional
        If True, return the result as a DataFrame. Default is True.
    kwargs : dict
        Additional keyword arguments to be passed to the `get_isochrones` function.
    ...
    ------
    ValueError
        If neither `Z` nor `MH` is provided, or if both are provided.
    """
    if Z is None and MH is None:
        raise ValueError("Either Z or MH must be provided.")
    if Z is not None and MH is not None:
